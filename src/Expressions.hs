...
--------------------- Level 4 --------------------
term3 :: ParsecT [Token] [MemCell] IO(Token)
term3 = try bin_term3 <|> una_term3

una_term3 :: ParsecT [Token] [MemCell] IO(Token)
una_term3 =  (do
...
--- funções considerando associatividade à esquerda                  
bin_term3 :: ParsecT [Token] [MemCell] IO(Token)
bin_term3 = do
                   n1 <- term4
                   result <- eval_term3 n1
                   return (result)

eval_term3 :: Token -> ParsecT [Token] [MemCell] IO(Token)
eval_term3 n1 = do
                    op <- multToken <|> divToken <|> modToken
                    n2 <- term4
...
--------------------- Level 5 --------------------
term4 :: ParsecT [Token] [MemCell] IO(Token)
term4 = try bin_term4 <|> una_term4

una_term4 :: ParsecT [Token] [MemCell] IO(Token)
una_term4 =  (do
                op <- powerToken
...
--- funções considerando associatividade à esquerda                  
bin_term4 :: ParsecT [Token] [MemCell] IO(Token)
bin_term4 = do
                   n1 <- intToken <|> variable <|> booleanToken 
                   result <- eval_term4 n1
                   return (result)

eval_term4 :: Token -> ParsecT [Token] [MemCell] IO(Token)
eval_term4 n1 = do
                    op <- powerToken
